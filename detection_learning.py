...
def compute_drowsiness_score(blink_rate, perclos, blink_rate_threshold=15, perclos_threshold=35):
...
"""paramètres phase d'apprentissage"""
...
while time.time() - learning_start <= learning_window:
    _, frame = cap.read()
    ...
    if closest_face is not None:
        ...
        if time.time() - start_time >= time_window:
            ...
            data_perclos.append(perclos)
            data_blink_rate.append(blink_count)
            blink_count = 0
            closed_count = 0
            frame_count = 0
            start_time = time.time()

        cv2.imshow("apprentissage", frame)
        cv2.moveWindow("apprentissage", 350, 150)
...
perclos_threshold = np.average(data_perclos)
blink_rate_threshold = np.average(data_blink_rate)
...
while True:
    ...
    if closest_face is not None:
        ...
        if time.time() - start_time >= time_window:
            perclos = closed_count / frame_count * 100
            score = compute_drowsiness_score(blink_count, perclos, blink_rate_threshold, perclos_threshold)
            playSon(score)
            blink_count = 0
            closed_count = 0
            frame_count = 0
